...
(async() => {
  print(chalk`{bgRed {bold.white [>] Follow User by Location (+Like and Comment)}}\n`);
  const questions = [{
    type: 'input',
    name: 'username',
  ...
  try{
    const {username, password, location, perExec, delayTime} = await inquirer.prompt(questions);
